...
namespace BreakTheBankTabs1063
{
    ...
    public class StateMain : IGameState
    {
        ...
        public AnimatedLabel animAnySameWin = null; 
        ...
        public int playFreeTabsWon=0;
        public decimal playBonusWon=0;
        // spin 
        public List<AnimatedLabel> Animations = new List<AnimatedLabel>();
        public AnimatedLabel animBlur = null;
        public AnimatedLabel[,] SpinningReelAnims = new AnimatedLabel[Definitions.reelWidth,Definitions.iconsViewRows];
        public bool[,] reelSpinning = new bool[3, 3];
        ...
        public override string OnUpdate(Keyboard keyboard, Mouse mouse, GameCore.Timing.Timer timer)
        {
            ...
            else
            {
                ...
                if(almMeterCycle.Check(Alarm.CheckType.RESET))
                {
                    ...
                    lblFreeTabs.Text = "   X "+Definitions.freeTabsPayTable[curFreeTabsCycle].repeatitions.ToString()+"\r\n"+
                        Definitions.freeTabsPayTable[curFreeTabsCycle].FreeTabs.ToString()+" tabs";
                }
            }
            ...
            if (Bingo.singleton.localSettings.IsSpin)
            {
                for (int i = 0; i < 3; i++)
                {
                    for (int j = 0; j < 3; j++)
                    {
                        if (SpinningReelAnims[i,j] != null)

                        {
                            if (SpinningReelAnims[i, j].IsPlaying())
                            {
                                if (i < 2)
                                {
                                    if (SpinningReelAnims[i + 1, j] == null && SpinningReelAnims[i, j].GetFrame() > 3)
                                    {
                                        SpinReel(i + 1);
                                    }

                                    if (SpinningReelAnims[i, j].GetFrame() > 2 && !iconsView.HideCol[i, j])
                                    {
                                        iconsView.HideCol[i, j] = true;
                                    }
                                }
                                else
                                {
                            ...
                            {
                                iconsView.HideCol[i, j] = false;
                                Animations.Remove(SpinningReelAnims[i, j]);
                                if (reelSpinning[i, j])
                                {
                                    reelSpinning[i, j] = false;
                                    if (soundsOn)
                                    {
                                        Sounds.reelStop.Stop(true);
                                        Sounds.reelStop.Play(false, true);
                                        if (i == 2)
                                        {
        ...
        public bool spinDone()
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                for (int j = 0; j < 3; j++)
                {
                    done &= (SpinningReelAnims[i, j] == null || !SpinningReelAnims[i, j].IsPlaying());
                }
            }
            return done;
        }

        public void clearReelAnims()
        {
            for (int i = 0; i < 3; i++)
            {
                for (int j = 0; j < 3; j++)
                {
                    SpinningReelAnims[i, j] = null;
                }
            }
        }
        ...
        public void savePlay()
        {
            if (Bingo.singleton.curPIN.PIN != Bingo.singleton.settings.demoPIN)
            {
                ...
                curPlay.data = DataFunctions.convertGameDataToString(Bingo.gameSystemID, freeTabsMode, iconsView.icons, playFreeTabsWon, playBonusWon);
